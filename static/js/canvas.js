...
var links = [
	{name:"ABOUT", x:width/4*3, y:height/4, link:"static/images/aboutSnip.PNG", linkName:"about.html"},
	{name:"CONTACT", x:width/1.7, y:height/4*3, link:"static/images/contactSnip.PNG", linkName:"contact.html"},
	{name:"MY PROJECTS", x:width/7, y:height/5, link:"static/images/projectsSnip.png", linkName:"projects.html"}
]
...
document.addEventListener("click", function(){
	for (var i = 0; i < links.length; i++) {
		if(Math.abs(mouse.x - links[i].x) <= linkDist && Math.abs(mouse.y - links[i].y) <= linkDist){
			location.href = links[i].linkName;
		}
	}
})
